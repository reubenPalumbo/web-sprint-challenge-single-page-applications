...
1. In 1-2 sentences, explain what React's `useRouteMatch` hook is used for.
   useRouteMatch is used to match a URL without using the Route rendering directly
1. How would you explain form validation to someone who has never programmed before?
   In order for the form to submited we need to make sure the things inputed in the text boxes are the things we are looking for. Using validation it makes sure we know what is in the boxes so that we can send it to be stored.
1. In 1-2 sentences, define end to end testing.
   End to end is where we use all part of the app to test things. Having the server, main app, and any APIs in use. We are testing for bugs of our code. Test need to make sure that if something goes wrong that it will show an error in the test, the point is to show errors, not make a test that passes 100%.
